...
async function run() {
  console.log("🚀 Connecting to Supabase and executing relations migration...");
  try {
    ...
    console.log("👉 Target Enum values:", mergedTypes);

    // 2. Add Foreign Key for category_id
    ...
    // 3. Add Foreign Key for brand_id
    ...
    // 4. Create ENUM type dynamically
    ...
    if (typeExists.length === 0) {
      // Build the CREATE TYPE query dynamically safely
